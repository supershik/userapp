...
const LiveOrderDetailScreen = ({ navigation, route }) => {
  ...
  const updateLiveOrders = () => {
    Toast.show('Pressed update button!');
    return;
  ...
  const PayBill = () => {
    ...
    // navigation.navigate('Cashfree', {orderId: orderRef, orderAmount: orderRoot.ordertotal, payload: payload})
    let pickDateTime = orderpickupdate + ' ' + orderpickuptime;
    navigation.navigate('Cashfree', {orderId: orderRef, orderAmount: orderRoot.ordertotal, orderpickuptime: pickDateTime});
  }
  
  const onPressRefund = () => {
    Toast.show('Pressed Refund!, if orderstatus equals paid');
  }

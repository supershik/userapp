...
const SearchScreen = ({ navigation, route }) => {
    ...
    navigation.setOptions({ 
      ...
      headerRight: () => (
        <View style={{flexDirection: 'row', alignSelf: "center", marginRight: 5}}>
          <TouchableOpacity
            onPress={() => navigation.navigate('CartFromCategoriesHome', {shopcategoryinfo: shopcategoryinfo, fromView: fromView})}>
            <Image
              source={addcart}
  ...
  const handleSearch = (query) => {
    ...
    if (query.length >= 3) {
      ...
      SHOPAPIKit.get('/product/search/products/shopcategory/' + shopcategoryinfo.shopcategoryid + '/' + query)
        .then(onSuccess)
        .catch(onFailure);
    }
  ...
  const onPressLatestProducts = () => {
    ...
    setLoading(true);
    setData([]);
    SHOPAPIKit.get('/product/latest/products/shopcategory/' + shopcategoryinfo.shopcategoryid)
      .then(onSuccess)
      .catch(onFailure);
  }
